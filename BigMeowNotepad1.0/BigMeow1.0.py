...
def about():
    messagebox.showinfo(title="BigMeow1.0", message="MyCat, keep it up")
...
class editor():
    def __init__(self, rt):
        ...
        self.t.title("BigMeow%d" % (len(t1) + 1))

        self.frm_file = tk.Frame(rt)
        self.frm_file.grid(row=0, column=0, padx=0, sticky=W)
        self.btn_open = Button(self.frm_file, text="Open", relief=GROOVE, command=self.openfile)
        self.btn_open.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_new = Button(self.frm_file, text="New", relief=GROOVE, command=self.neweditor)
        self.btn_new.pack(side=LEFT, padx=5, fill=BOTH)
        self.btn_save = Button(self.frm_file, text="Save", relief=GROOVE, command=self.savefile)
        self.btn_save.pack(side=LEFT, padx=5, fill=BOTH)
        self.btn_saveas = Button(self.frm_file, text="Save As", relief=GROOVE, command=self.saveasfile)
        self.btn_saveas.pack(side=LEFT, padx=5, fill=BOTH)
        self.btn_exit = Button(self.frm_file, text="Exit", relief=GROOVE, command=self.close)
        self.btn_exit.pack(side=RIGHT, padx=5, fill=BOTH)

        self.frm_edit = tk.Frame(rt)
        self.frm_edit.grid(row=0, column=1, padx=1, sticky=W)
        self.btn_copy = Button(self.frm_edit, text="Copy")
        self.btn_copy.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_cut = Button(self.frm_edit, text="Cut")
        self.btn_cut.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_paste = Button(self.frm_edit, text="Paste")
        self.btn_paste.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_find = Button(self.frm_edit, text="Find")
        self.btn_find.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_allselect = Button(self.frm_edit, text="Select All")
        self.btn_allselect.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_font = Button(self.frm_edit, text="Font Style")
        self.btn_font.pack(side=LEFT, padx=5, fill=BOTH, expand=1)
        self.btn_color = Button(self.frm_edit, text="Font Color")
        self.btn_color.pack(side=LEFT, padx=5, fill=BOTH, expand=1)

        self.bar = tk.Menu(rt)
        self.filem = tk.Menu(self.bar)
        self.filem.add_separator()
        self.filem.add_command(label="New", command=self.neweditor, accelerator="     Ctr +N")
        self.filem.bind_all("<Control-n>", self.neweditor)
        self.filem.add_separator()
        self.filem.add_command(label="Open", command=self.openfile, accelerator="     Ctr + O")
        self.filem.bind_all("<Control-o>", self.openfile)
        self.filem.add_separator()
        self.filem.add_command(label="Save", command=self.savefile, accelerator="     Ctr +S ")
        self.filem.bind_all("<Control-s>", self.savefile)
        self.filem.add_separator()
        self.filem.add_command(label="Save As", command=self.saveasfile, accelerator="     Ctr + D ")
        self.filem.bind_all("<Control-d>", self.saveasfile)

        self.filem.add_command(label="Close", command=self.close, accelerator="     F4")
        self.filem.bind_all("<F4>", self.close)
        self.filem.add_separator()
        self.filem.add_command(label="Exit", command=die, accelerator="     ESC")
        self.filem.bind_all("<Escape>", die)

        self.editm = tk.Menu(self.bar)
        self.editm.add_separator()
        self.editm.add_command(label="Copy", command=self.copy, accelerator=" " * 10 + "Ctr + C")
        self.editm.bind_all("<Control-c>", self.copy)
        self.editm.add_separator()
        self.editm.add_command(label="Paste", command=self.paste, accelerator=" " * 10 + "Ctr + V")
        self.editm.bind_all("<Control-v>", self.paste)
        self.editm.add_separator()
        self.editm.add_command(label="Cut", command=self.cut, accelerator=" " * 10 + "Ctr + X")
        self.editm.bind_all("<Control-x>", self.cut)
        self.editm.add_separator()
        self.editm.add_command(label="Delete", command=self.delete_text, accelerator=" " * 10 + "Delete")
        self.editm.bind_all("<Delete>", self.delete_text)
        self.editm.add_separator()
        self.editm.add_command(label="Find", command=self.find_char, accelerator=" " * 10 + "Ctr +F")
        ...
        self.formm.add_command(label="Font Color", command=self.color_it, accelerator=" " * 10 + "Alt + C")
        self.formm.bind_all("<Alt-f>", self.color_it)
        self.formm.add_separator()
        self.formm.add_command(label="Font Style", command=self.font_it, accelerator=" " * 10 + "Alt + F")
        ...
        self.bar.add_cascade(label="Style", menu=self.formm)
    ...
    def openfile(self, event=None):
        oname = filedialog.askopenfilename(filetypes=[("Open File", "*.txt")])
        if oname:
    ...
    def savefile(self, event=None):
        if os.path.isfile(self.t.title()):
            ...
            sname = filedialog.asksaveasfilename(title="Save MyCat", filetypes=[("Save File", "*.txt")],
    ...
    def saveasfile(self, event=None):
        sname = filedialog.asksaveasfilename(title="Save MyCat", filetypes=[("Save File", "*.txt")], defaultextension=".txt")
        if sname:
            ofp = open(sname, "w")
            ofp.write(self.st.get(1.0, tk.END))
            ofp.flush()
    ...
    def find_char(self, event=None):
        target = simpledialog.askstring("BigMeow1.0", "寻找字符串")
        if target:
            ...
            while pos_line <= end_line:
                if pos_line == end_line and pos_column + length > end_column:
                    ...
                    if where:
                        ...
                        again = messagebox.askokcancel(title="Continue to inquire?")
                        if again:
                            pos_line = int(where1[0])
                            pos_column = int(sele_end_col)
                        else:
                            aa = messagebox.showinfo(title="You finally gave up on me", message="You gave me up--!")
                            if aa:
                                sys.exit()
    ...
    def font_it(self, event=None):
        ...
        self.t_font.title("Font selection panel")
        self.label_size = Label(self.t_font, text="Font Size")
        self.label_shape = Label(self.t_font, text="Font Shape")
        self.label_font = Label(self.t_font, text="Font Style")
        self.label_weight = Label(self.t_font, text="Font Weight")
        ...
        self.labFra_display = LabelFrame(self.t_font, text="Font style demo area")
        self.labFra_display.grid(row=2, column=0, pady=4)
        self.lab_display = Label(self.labFra_display, text="I am here")
        self.lab_display.pack()

        self.btn_ok = Button(self.t_font, text="", width=10, height=2, command=self.change)
